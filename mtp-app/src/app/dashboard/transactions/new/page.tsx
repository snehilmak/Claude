...
export default function NewTransactionPage() {
  ...
  const [activeMode, setActiveMode] = useState<'all' | 'single'>('all');
  const [activeLocationName, setActiveLocationName] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  const [form, setForm] = useState({
    customerId: '', recipientName: '', recipientPhone: '', recipientCity: '',
    recipientCountry: 'MX', companyId: '', sendAmount: '', exchangeRate: '',
    fee: '', paymentMethod: 'CASH', referenceNumber: '', controlNumber: '',
    purposeOfTransfer: 'Family Support', sourceOfFunds: 'Employment',
    notes: '', locationId: '',
  });

  const receiveAmount = form.sendAmount && form.exchangeRate
    ? (parseFloat(form.sendAmount) * parseFloat(form.exchangeRate)).toFixed(2) : '';
  const totalCollected = form.sendAmount && form.fee
    ? (parseFloat(form.sendAmount) + parseFloat(form.fee)).toFixed(2) : form.sendAmount;
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (loc.active?.mode === 'single') {
        setActiveMode('single');
        setActiveLocationName(loc.active.locationName);
      } else {
        setActiveMode('all');
        // Default to first location
        if (loc.locations?.length > 0) {
          setForm((p) => ({ ...p, locationId: loc.locations[0].id }));
        }
      }
    });
  }, []);
    ...
    try {
      const res = await fetch('/api/transactions', {
        ...
        body: JSON.stringify({
          ...
          receiveAmount,
          totalCollected,
          locationId: activeMode === 'all' ? form.locationId : undefined,
        }),
      });
      if (!res.ok) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <div className={`flex items-center gap-3 rounded-lg px-4 py-3 text-sm ${
        activeMode === 'single' ? 'bg-blue-50 border border-blue-200' : 'bg-gray-50 border border-gray-200'
      }`}>
        <MapPin className="h-4 w-4 text-blue-600 flex-shrink-0" />
        {activeMode === 'single' ? (
          <span className="text-blue-800">
            Transfer will be recorded at: <strong>{activeLocationName}</strong>
          </span>
        ) : (
          <div className="flex items-center gap-3 flex-1">
            <span className="text-gray-700">Record transfer at:</span>
            <select
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <p className="text-xs text-blue-600 hover:underline cursor-pointer" onClick={() => router.push('/dashboard/customers/new')}>
              + Add new customer
            </p>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              ...
              <div className="space-y-1">
                ...
                <div className="rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm font-medium text-gray-700">{receiveAmount || '0.00'}</div>
              </div>
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Select label="Purpose of Transfer" id="purposeOfTransfer" value={form.purposeOfTransfer} onChange={set('purposeOfTransfer')}>
              <option>Family Support</option><option>Business Payment</option>
              <option>Gift</option><option>Medical Expenses</option>
            ...
            <Select label="Source of Funds" id="sourceOfFunds" value={form.sourceOfFunds} onChange={set('sourceOfFunds')}>
              <option>Employment</option><option>Business</option>
              <option>Savings</option><option>Pension / Retirement</option><option>Other</option>
            </Select>
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button type="submit" disabled={loading}>{loading ? 'Processing...' : 'Create Transfer'}</Button>
        </div>
      </form>
    </div>
  );
